...
local function makeTrack(instantHighQuality, ignoreCollision)
  ...
  if #subPieces[subTrackIndex] == 0 then
    -- M.unloadAll() -- commented out game code so multiple tracks can be spawned & removed
    M.initTrack()
  end
  local track = subTracks[subTrackIndex]
...
local function initTrack(noUnload)
  -- if not noUnload then  -- commented out game code so multiple tracks can be spawned & removed
  --   M.unloadAll()
  -- end
...
--   M.removeTrack() --This line is removed from the games lua, otherwise it's not possible to spawn multiple tracks
  currentSelected = 1
  -- subPieces = {{}} --This line is removed from the games lua, otherwise it's not possible to spawn multiple tracks
  -- subTrackPositions = {{}}  --This line is removed from the games lua, otherwise it's not possible to spawn multiple tracks
  -- subTracks = {{}} --This line is removed from the games lua, otherwise it's not possible to spawn multiple tracks
  intersections = {}
  ...
  for k,v in pairs(import.subPieces) do subPieces[k] = v end --This line is added from the games lua, otherwise it's not possible to spawn multiple tracks
  -- subPieces = import.subPieces --This line is removed from the games lua, otherwise it's not possible to spawn multiple tracks
...
local function onCollectScreenshotMetadata(metadata)
